...
class OTFSChannelEstimator:
    ...
    def generate_center_pilot_frame(self, pilot_density=0.25, pilot_spacing=2, edge_guard=1):
        """ pilot_density 
            pilot_spacing: pilot to pilot
            edge_guard: pilot to edge
        """
        
        # center(consider Doppler Shift,place the pilot at center)
        center_m = self.M // 2
        center_n = self.N // 2
        
        # pilot_density（not actually num）
    ...
    def improved_ls_estimation(self, Y, pilot_pos, pilot_syms):
        """Improved LS estimation  element wise """
        Y_flat = Y.T.flatten()
    ...
    def generate_training_dataset(self, n_samples=10000, snr_range=(0, 35), speed_range=(5, 600),
        ...
        """Generate training dataset with dual guard parameters"""
        ...
        for i in range(n_samples):
            ...
            # Generate center pilot frame with dual guard parameters
    ...
    def visualize_channel_estimation_results(self, n_vis=1, pilot_spacing=2, edge_guard=1):
        """visualize_channel_estimation_results"""
        
        print(f"Visualizing channel estimation results with current parameters...")
        ...
        for vis_idx in range(n_vis):
            ...
            ax4.set_title(f'DNN Estimation ({display_size}x{display_size})')
            ...
            plt.suptitle(f'Channel Estimation Comparison\nSNR={snr_db}dB, Speed={max_speed}km/h, {channel_type}\nPilot Spacing={pilot_spacing}, Edge Guard={edge_guard}', 
                        fontsize=14)
            plt.tight_layout()
            plt.savefig(f'channel_estimation_{vis_idx}_spacing{pilot_spacing}_edge{edge_guard}.png', 
            ...
            print(f"Sample {vis_idx+1} Performance (Spacing={pilot_spacing}, Edge={edge_guard}):")
    ...
    def comprehensive_comparison(self, n_train=20000, n_test=1000, epochs=100, batch_size=64, 
                               pilot_spacing=2, edge_guard=1):
        """Run comprehensive comparison with current parameters"""
        ...
        print(f"\n=== Using Current Parameters (Pilot Spacing={pilot_spacing}, Edge Guard={edge_guard}) ===")
    ...
    def plot_results(self, result):
        """Plot comparison results with current parameters"""
        ...
        axes[1].semilogy(test_results['snr_values'], test_results['mse_dnn'], '^-',
                      label=f'DNN (S={pilot_spacing}, E={edge_guard})', linewidth=2, markersize=8)
        axes[1].set_xlabel('SNR (dB)')
        axes[1].set_ylabel('MSE')
        axes[1].set_title(f'MSE vs SNR')
        axes[1].legend()
        ...
        axes[2].set_title(f'NMSE vs SNR')
        axes[2].legend()
        axes[2].grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(f'otfs_results_spacing{pilot_spacing}_edge{edge_guard}.png', 
...
def main():
    ...
    print("=== OTFS Channel Estimation (Current Parameters) ===")
    ...
    # Run comprehensive comparison with current parameters
    print(f"Running comprehensive comparison with current parameters...")
    ...
    with open(f'otfs_results_S{PILOT_SPACING}_E{EDGE_GUARD}.pkl', 'wb') as f:
        pickle.dump(result, f)
    
    if estimator.dnn_model_full is not None:
        estimator.dnn_model_full.save(f'otfs_dnn_S{PILOT_SPACING}_E{EDGE_GUARD}.h5')
    
    # Performance summary
    print("\n=== Performance Summary (Current Parameters) ===")
    test_results = result['test_results']
    
    if 20 in test_results['snr_values']:
        idx = test_results['snr_values'].index(20)
        print(f"\nAt 20dB SNR (Pilot Spacing={PILOT_SPACING}, Edge Guard={EDGE_GUARD}):")
    ...
    print(f"\nResults saved to:")
    print(f"  - Main results: otfs_results_S{PILOT_SPACING}_E{EDGE_GUARD}.pkl")
    print(f"  - Model: otfs_dnn_S{PILOT_SPACING}_E{EDGE_GUARD}.h5")
    print(f"  - Plots: Various PNG files")
